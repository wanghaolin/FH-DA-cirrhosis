...
fh_params_config = {
    # 基于模糊超盒的参数搜索空间
    'base': base_shared_params.copy(),
    # 基于模糊椭球体的参数搜索空间
    'he': {
        **base_shared_params,
        'gamma': (0.01, 50),  # 缩小gamma搜索范围，避免极端值
        'reg_lambda': (1e-6, 1e-4),  # 缩小正则化参数搜索范围，基于经验值
    },
    # 基于模糊球体的参数搜索空间
    'sphere': {
        **base_shared_params,
...
fh_models = {
    ...
    #     'class': FHDES_Allboxes_vector,
    #     'params': fh_params_config['base']
    # },
    # 基于马氏距离的超椭球体模型
    'FHDES_Allboxes_vector_he': {
        'class': FHDES_Allboxes_vector_he,
        'params': fh_params_config['he']
    },
    # 基于超球体模型
    'FHDES_Allboxes_vector_sphere': {
        'class': FHDES_AllBoxes_vector_sphere,
        'params': fh_params_config['sphere']
    },
    # 基于超矩形模型
    'FHDES_Allboxes_vector_rectangle': {
        'class': FHDES_AllBoxes_vector_rectangle,
        'params': fh_params_config['rectangle']
    },

    # 'FHDES_JFB_vector': {
    #     'class': FHDES_JFB_vector,
    #     'params': fh_params_config['base']
    # },
    # 'FHDES_JFB_vector_he': {
    #     'class': FHDES_JFB_vector_he,
    #     'params': fh_params_config['he']
    # },
    # # 基于超球体模型
    # 'FHDES_JFB_vector_sphere': {
    #     'class': FHDES_JFB_vector_sphere,
    #     'params': fh_params_config['sphere']
    # },
    # # 基于超矩形模型
    # 'FHDES_JFB_vector_rectangle': {
    #     'class': FHDES_JFB_vector_rectangle,
    #     'params': fh_params_config['rectangle']
    # },
}
    ...
    for param_name, param_range in model_info['params'].items():
        if isinstance(param_range, tuple):
            if isinstance(param_range[0], int):
                ...
                # 增加浮点数采样精度
                # 对于gamma和reg_lambda参数，使用对数空间采样
                if param_name in ['gamma', 'reg_lambda', 'sigma', 'beta']:
                    params[param_name] = trial.suggest_float(param_name, param_range[0], param_range[1], log=True)
                elif param_name == 'theta':
                    # 对于theta参数，使用对数空间采样，不设置step
                    params[param_name] = trial.suggest_float(param_name, param_range[0], param_range[1], log=True)
                elif param_name == 'mu':
                    # 对于mu参数，使用均匀采样，步长为0.05
        ...
        # 确保参数值不会太长，避免数据库存储问题
        if isinstance(params[param_name], float):
            # 限制浮点数精度
            params[param_name] = round(params[param_name], 6)
        elif isinstance(params[param_name], str) and len(params[param_name]) > 100:
            # 截断过长的字符串参数
            params[param_name] = params[param_name][:100]
...
def optimize_fh_models(previous_study_dir=None):
    ...
    result_dir = f'best_params/hyperellipsoid/{timestamp}'
    ...
    for model_name in fh_models:
        ...
        # 保存最佳参数
        all_best_params[model_name] = study.best_params
    ...
    # 保存最优参数到文件
    with open(f'{result_dir}/ACLF_fh_best_params.json', 'w', encoding='utf-8') as f:
        json.dump(all_best_params, f, indent=4)
    
    # 同时更新主要参数文件
    with open('best_params/hyperellipsoid/ACLF_fh_best_params.json', 'w', encoding='utf-8') as f:
        json.dump(all_best_params, f, indent=4)
...
def extract_best_params_from_studies(studies_folder, output_folder):
    ...
    # 汇总所有模型的最优参数
    summary_path = os.path.join(output_folder, "all_best_params.json")
...
if __name__ == "__main__":
    ...
    # previous_study_dir = 'best_params/hyperellipsoid/20251006_211514'
    ...
    # # 解析study文件并提取最优参数
    # studies_folder = 'best_params/hyperellipsoid/20251006_211514'  # 指定study.pkl所在文件夹
    # output_folder = 'best_params/hyperellipsoid/20251006_211514'  # 指定输出文件夹
