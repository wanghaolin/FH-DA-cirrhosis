...
class DESFHMW_allboxes_vector_sphere_clustering(BaseDES):
    ...
    def __init__(self, pool_classifiers=None,
        ...
        super(DESFHMW_allboxes_vector_sphere_clustering, self).__init__(pool_classifiers=pool_classifiers,
                                    with_IH=with_IH,
                                    safe_k=safe_k,
                                    IH_rate=IH_rate,
    ...
    def _adjust_boxes_by_density(self):
        ...
        for clsr_idx in range(len(self.HBoxes)):
            ...
            for box_idx in range(len(hboxC)):
                # 计算超盒中心到每个簇中心的距离
                distances_to_centers = cdist([hboxC[box_idx]], self.cluster_centers_)[0]
                ...
                # 调整超盒大小：密度高的区域超盒更小
                hboxR[box_idx] *= adjustment_factor
        ...
        for i in range(n_queries):
            ...
            for clsr_idx in range(self.n_classifiers_):
                ...
                for box in clsr_boxes:
                    ...
                    for box_idx in range(len(hboxC)):
                        # 计算超盒中心到各簇中心的距离
                        box_distances = cdist([hboxC[box_idx]], self.cluster_centers_)[0]
                ...
                # 结合基本竞争力和增强因子，使用gamma参数调整权重
                enhanced_competences[i, clsr_idx] = (basic_competences[i, clsr_idx] * (1 - self.gamma) + 
                                                    clsr_enhancement * self.gamma)
    ...
    def fit(self, X, y):
        super(DESFHMW_allboxes_vector_sphere_clustering, self).fit(X, y)
    ...
    def estimate_competence(self, query, neighbors=None, distances=None, predictions=None):
        ...
        if self.cluster_centers_ is None:
            scaler = preprocessing.MinMaxScaler()
            competences_ = scaler.fit_transform(basic_competences)
            return competences_
