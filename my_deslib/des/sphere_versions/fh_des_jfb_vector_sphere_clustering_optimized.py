...
class FHDES_JFB_vector_sphere_clustering_optimized(FHDES_JFB_vector_sphere):
    ...
    def __init__(self, pool_classifiers=None,
        ...
        # 调用父类初始化
        super(FHDES_JFB_vector_sphere_clustering_optimized, self).__init__(
        ...
        self.cluster_sphere_adjustments_ = None # 每个簇的超球调整因子
        self.pca_ = None  # PCA模型
        self.cluster_bandwidths_ = []  # 每个簇的自适应带宽
    
    def fit(self, X, y):
        # 调用父类fit方法
        super(FHDES_JFB_vector_sphere_clustering_optimized, self).fit(X, y)
        ...
        self._calculate_sphere_adjustments()
        
        # 基于聚类和密度调整超球
        self._adjust_spheres_by_density()
    ...
    def _calculate_sphere_adjustments(self):
        """计算每个簇的超球调整因子"""
        # 添加小常数确保数值稳定性
        ...
        # 计算每个簇的平均密度
        self.cluster_sphere_adjustments_ = np.zeros(self.n_clusters)
        for cluster_idx in range(self.n_clusters):
            cluster_mask = self.cluster_labels_ == cluster_idx
            if np.any(cluster_mask):
                # 密度越高，调整因子越小（使超球更紧凑）
                avg_density = np.mean(normalized_densities[cluster_mask])
                self.cluster_sphere_adjustments_[cluster_idx] = 1.0 - (0.5 * avg_density)  # 范围在0.5到1.0之间
            else:
                self.cluster_sphere_adjustments_[cluster_idx] = 1.0
    
    def _adjust_spheres_by_density(self):
        """基于聚类和密度信息调整超球形状"""
        X_dsel = self.DSEL_data_
        
        # 对每个分类器的超球进行调整
        for clsr_idx in range(len(self.HBoxes)):
            sphere = self.HBoxes[clsr_idx]
            classifier_idx = sphere["clsr"]
            hboxC = sphere["Center"]
            hboxR = sphere["Radius"]
            
            # 为每个超球找到最相关的簇
            for sphere_idx in range(len(hboxC)):
                # 计算超球中心
                sphere_center = hboxC[sphere_idx]
                
                # 找到离超球中心最近的簇
                # 如果使用了PCA，需要将超球中心也投影到PCA空间
                if hasattr(self, 'DSEL_data_pca_') and self.pca_ is not None:
                    sphere_center_pca = self.pca_.transform(sphere_center.reshape(1, -1))[0]
                    distances_to_centers = cdist([sphere_center_pca], self.cluster_centers_)[0]
                else:
                    distances_to_centers = cdist([sphere_center], self.cluster_centers_)[0]
                
                nearest_cluster = np.argmin(distances_to_centers)
                
                # 使用该簇的调整因子调整超球
                adjustment_factor = self.cluster_sphere_adjustments_[nearest_cluster]
                
                # 调整超球大小：密度高的区域超球更小
                hboxR[sphere_idx] *= adjustment_factor
    ...
    def estimate_competence(self, query, neighbors=None, distances=None, predictions=None):
        ...
        basic_competences = super(FHDES_JFB_vector_sphere_clustering_optimized, self).estimate_competence(
        ...
        for i in range(n_queries):
            ...
            for cluster_idx in range(self.n_clusters):
                if self.cluster_kdes_[cluster_idx] is not None:
                    if self.density_estimation == 'knn_density':
                        ...
                        log_density = kde.score_samples(query_point_transformed)
                        query_densities[cluster_idx] = np.exp(log_density)
            
            # 归一化查询点密度
            max_density = np.max(query_densities)
            if max_density > 1e-8:
                normalized_query_densities = query_densities / max_density
            else:
                normalized_query_densities = np.ones_like(query_densities) * (1.0 / self.n_clusters)
            ...
            for clsr_idx in range(self.n_classifiers_):
                ...
                for sphere in clsr_spheres:
                    ...
                    # 计算查询点与当前超球的隶属度
                    sphere_membership = self.membership_spheres(hboxC, hboxR, query_point)
                    query_memberships.append(sphere_membership)
                
                # 合并所有超球的隶属度
                if query_memberships:
                    # 取最大值或平均值作为该分类器的基本竞争力
                    clsr_membership = np.max(np.concatenate(query_memberships, axis=0))
                else:
                    clsr_membership = 0.0
                ...
                for sphere in clsr_spheres:
                    ...
                    # 计算超球中心到所有簇中心的距离
                    if self.pca_ is not None:
                        hboxC_pca = self.pca_.transform(hboxC)
                        sphere_to_clusters = cdist(hboxC_pca, self.cluster_centers_)
                    else:
                        sphere_to_clusters = cdist(hboxC, self.cluster_centers_)
                    
                    # 找到超球与最近簇的距离和索引
                    min_dist_per_sphere = np.min(sphere_to_clusters, axis=1)
                    nearest_cluster_per_sphere = np.argmin(sphere_to_clusters, axis=1)
                    
                    # 计算每个超球的相关性
                    sphere_relevance = 0.0
                    for j in range(len(hboxC)):
                        cluster_idx = nearest_cluster_per_sphere[j]
                        # 相关性基于超球到簇的距离和簇的密度
                        dist_weight = 1.0 / (min_dist_per_sphere[j] + self.epsilon)
                        density_weight = normalized_query_densities[cluster_idx]
                        sphere_relevance += (self.cluster_distance_weight * dist_weight + 
                                            self.density_weight * density_weight)
                    
                    clsr_relevance += sphere_relevance
                
                # 归一化相关性
                if self.normalize_correlations and len(clsr_spheres) > 0:
                    clsr_relevance /= len(clsr_spheres)
                
                # 计算增强竞争力
                # 基本竞争力 + 增强因子
                enhancement_factor = clsr_relevance
                enhanced_competence = (self.basic_weight * clsr_membership + 
                                      self.enhancement_weight * enhancement_factor)
                
                enhanced_competences[i, clsr_idx] = enhanced_competence
        
        # 归一化增强竞争力
        for i in range(n_queries):
            max_competence = np.max(enhanced_competences[i])
            if max_competence > 1e-8:
                enhanced_competences[i] /= max_competence
            else:
                enhanced_competences[i] = np.ones_like(enhanced_competences[i]) * (1.0 / self.n_classifiers_)
