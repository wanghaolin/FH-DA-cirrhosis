...
class FHDES_AllBoxes_vector_sphere(FHDES_Allboxes_vector):
    ...
    def __init__(self, pool_classifiers=None,
        ...
        # 调用父类初始化，使用正确的参数
        super(FHDES_AllBoxes_vector_sphere, self).__init__(pool_classifiers=pool_classifiers,
                                    with_IH=with_IH,
                                    safe_k=safe_k,
                                    IH_rate=IH_rate,
    ...
    def is_expandable(self, hboxC, hboxR, sphereInd, x):
        # 计算扩展后的半径
        current_radius = hboxR[sphereInd]
        # 确保current_radius是标量，处理不同形状的数组
        if isinstance(current_radius, np.ndarray):
            # 获取第一个元素，或者如果是多维数组则使用适当的索引
            current_radius = current_radius[0] if current_radius.size > 0 else 0
            # 再次检查是否仍为数组
            if isinstance(current_radius, np.ndarray):
                current_radius = current_radius.item() if current_radius.size == 1 else 0
        
        distance = np.linalg.norm(hboxC[sphereInd] - x)
        new_radius = max(float(current_radius), distance)
        # 检查是否超过阈值
    ...
    def fit(self, X, y):
        super(FHDES_AllBoxes_vector_sphere, self).fit(X, y)
    ...
    def estimate_competence(self, query, neighbors=None, distances=None, predictions=None):
        ...
        # 确保HBoxes被正确初始化
        if not hasattr(self, 'HBoxes') or not self.HBoxes:
            return highest_mems

        Xq = query.reshape(1, len(query), self.n_features_)

        for clsr_dict in self.HBoxes:
            # 确保字典包含必要的键
            if "Center" not in clsr_dict or "Radius" not in clsr_dict or "clsr" not in clsr_dict:
                continue
            
            hboxC = clsr_dict["Center"]
            hboxR = clsr_dict["Radius"]
            clsr_index = clsr_dict["clsr"]
            ...
            if len(hboxC) > 1:
                ...
                for i in range(0,len(query)):
                    if clsrSpheres_m[b1[i],i]==1 : # if the query sample is located inside or near to the box
                        highest_mems[i, clsr_index] = 1
                    else:
                        highest_mems[i, clsr_index] = clsrSpheres_m[b1[i],i] *0.7 + clsrSpheres_m[b2[i],i]*0.3

            else:  # In case that we have only one hyperbox for the classifier
                for i in range(0, len(query)):
                    highest_mems[i, clsr_index] = clsrSpheres_m[0, i]
