...
class FHDES_JFB_vector_rectangle_clustering_optimized(FHDES_JFB_vector_rectangle):
    ...
    def _adjust_boxes_by_density(self):
        ...
        for clsr_idx in range(len(self.HBoxes)):
            ...
            # 为每个超盒找到最相关的簇
            for box_idx in range(len(hboxMin)):
                # 计算超盒中心
                box_center = (hboxMin[box_idx] + hboxMax[box_idx]) / 2
                ...
                # 调整超盒大小：密度高的区域超盒更小
                box_size = (hboxMax[box_idx] - hboxMin[box_idx]) * adjustment_factor
                new_center = (hboxMin[box_idx] + hboxMax[box_idx]) / 2
                hboxMin[box_idx] = new_center - box_size / 2
                hboxMax[box_idx] = new_center + box_size / 2
    ...
    def estimate_competence(self, query, neighbors=None, distances=None, predictions=None):
        ...
        basic_competences = super(FHDES_JFB_vector_rectangle_clustering_optimized, self).estimate_competence(
        ...
        for i in range(n_queries):
            ...
            for clsr_idx in range(self.n_classifiers_):
                ...
                for box in clsr_boxes:
                    ...
                    # 计算查询点与当前超盒的隶属度
                    box_membership = self.membership_boxes(hboxMin, hboxMax, query_point)
                    query_memberships.append(box_membership)
                ...
                for box in clsr_boxes:
                    ...
                    # 计算超盒中心到所有簇中心的距离
                    box_centers = (hboxMin + hboxMax) / 2
                    
                    if hasattr(self, 'pca_'):
                        box_centers_pca = self.pca_.transform(box_centers)
                        box_to_clusters = cdist(box_centers_pca, self.cluster_centers_)
                    else:
                        box_to_clusters = cdist(box_centers, self.cluster_centers_)
                    
                    # 找到超盒与最近簇的距离和索引
                    min_dist_per_box = np.min(box_to_clusters, axis=1)
                    nearest_cluster_per_box = np.argmin(box_to_clusters, axis=1)
                    
                    # 计算每个超盒的相关性
                    box_relevance = 0.0
                    for j in range(len(hboxMin)):
                        cluster_idx = nearest_cluster_per_box[j]
                        # 相关性基于超盒到簇的距离和簇的密度
                        dist_weight = 1.0 / (min_dist_per_box[j] + self.epsilon)
                        density_weight = normalized_query_densities[cluster_idx]
                        box_relevance += (self.cluster_distance_weight * dist_weight + 
                                        self.density_weight * density_weight)
                    
                    clsr_relevance += box_relevance
                
                # 归一化相关性
                if self.normalize_correlations and len(clsr_boxes) > 0:
                    clsr_relevance /= len(clsr_boxes)
