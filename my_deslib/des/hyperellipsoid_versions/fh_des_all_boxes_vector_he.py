...
class FHDES_Allboxes_vector_he(BaseDES):
    ...
    def __init__(self, pool_classifiers=None,
        ...
        super(FHDES_Allboxes_vector_he, self).__init__(pool_classifiers=pool_classifiers,
                                    with_IH=with_IH,
                                    safe_k=safe_k,
                                    IH_rate=IH_rate,
    ...
    def fit(self, X, y):
        super(FHDES_Allboxes_vector_he, self).fit(X, y)
    ...
    def setup_hyperellipsoids(self, classifier):
        ...
        for ind, X in enumerate(selected_samples):
            ...
            # 检查样本的隶属度是否已经足够高
            memberships = self.membership_ellipsoids(centers, cov_matrices, X.reshape(1, 1, -1))
            ...
            for ind_ellipsoid in sorted_indexes:
                if self.thetaCheck and self.doContraction:
                    ...
                    # 只检查theta，不进行收缩
                    # 这里简化处理，实际应用中可能需要更复杂的判断
                    # 更新椭球体
                    new_center, new_cov = self.update_ellipsoid(centers, cov_matrices, ind_ellipsoid, X)
                    # 检查更新后的协方差矩阵是否在允许的范围内
                    if np.trace(new_cov) < self.theta * self.n_features_ * 10:  # 简化的theta检查
                        centers[ind_ellipsoid] = new_center
                        cov_matrices[ind_ellipsoid] = new_cov
                        expanded = True
                        break
