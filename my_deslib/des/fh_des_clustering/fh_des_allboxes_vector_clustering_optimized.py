...
class FHDES_Allboxes_vector_clustering_optimized(FHDES_Allboxes_vector):
    ...
    def __init__(self, pool_classifiers=None,
        ...
        # 调用父类初始化
        super(FHDES_Allboxes_vector_clustering_optimized, self).__init__(
    ...
    def fit(self, X, y):
        # 调用父类fit方法
        super(FHDES_Allboxes_vector_clustering_optimized, self).fit(X, y)
    ...
    def _adjust_boxes_by_density(self):
        ...
        for clsr_idx, hbox_dict in enumerate(self.HBoxes):
            ...
            for box_idx in range(len(hboxV)):
                ...
                new_size = box_size * adjustment_factor
                
                # 更新超盒范围，确保不小于最小值
                min_size = np.ones_like(new_size) * 1e-8
                new_size = np.maximum(new_size, min_size)
                
                new_hboxV = box_center - new_size / 2
                new_hboxW = box_center + new_size / 2
                
                # 更新超盒
                hboxV[box_idx] = new_hboxV
                hboxW[box_idx] = new_hboxW
    ...
    def estimate_competence(self, query, neighbors=None, distances=None, predictions=None):
        """基于聚类和密度的竞争力估计"""
        # 调用父类的基本竞争力估计
        basic_competences = super(FHDES_Allboxes_vector_clustering_optimized, self).estimate_competence(query, neighbors, distances, predictions)
