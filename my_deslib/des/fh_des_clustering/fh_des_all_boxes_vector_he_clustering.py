...
class FHDES_AllBoxes_vector_he_clustering(FHDES_Allboxes_vector_he):
    ...
    def __init__(self, pool_classifiers=None,
        ...
        # 调用父类初始化
        super(FHDES_AllBoxes_vector_he_clustering, self).__init__(
    ...
    def fit(self, X, y):
        ...
        """
        super(FHDES_AllBoxes_vector_he_clustering, self).fit(X, y)
    ...
    def estimate_competence(self, query, neighbors=None, distances=None, predictions=None):
        ...
        basic_competences = super(FHDES_AllBoxes_vector_he_clustering, self).estimate_competence(
    ...
    def FMM_train(self, classifier_ind):
        # 调用父类的FMM_train方法构建基础椭球体
        centers, cov_matrices = super(FHDES_AllBoxes_vector_he_clustering, self).FMM_train(classifier_ind)
        
        # 注意：椭球体的调整在_adjust_boxes_by_density方法中进行，这里保持父类的实现不变
