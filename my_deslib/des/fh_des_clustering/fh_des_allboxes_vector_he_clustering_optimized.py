...
class FHDES_Allboxes_vector_he_clustering_optimized(FHDES_Allboxes_vector_he):
    ...
    def __init__(self, pool_classifiers=None,
        ...
        # 调用父类初始化
        super(FHDES_Allboxes_vector_he_clustering_optimized, self).__init__(
            ...
            DSEL_perc=DSEL_perc,
            HyperEllipsoids=HyperEllipsoids,
            theta=theta,
            mu=mu,
        ...
        self.cluster_ellipsoid_adjustments_ = None # 每个簇的椭球体调整因子
    ...
    def fit(self, X, y):
        ...
        训练流程：
        1. 聚类和密度估计
        2. 对每个分类器，基于聚类和局部密度优化椭球体大小
        3. 将最优椭球体参数保存到HEllipsoids
        4. 后续距离和隶属函数仍用父类的马氏距离和高斯隶属函数
        """
        super(FHDES_Allboxes_vector_he_clustering_optimized, self).fit(X, y)
        ...
        # 基于聚类和密度调整椭球体
        self._adjust_ellipsoids_by_density()
    ...
    def _calculate_ellipsoid_adjustments(self):
        """计算每个簇的椭球体调整因子"""
        # 归一化密度
        ...
        # 计算每个簇的平均密度和距离
        self.cluster_ellipsoid_adjustments_ = np.zeros(self.n_clusters)
        ...
        for cluster_idx in range(self.n_clusters):
            ...
            if np.any(cluster_mask):
                ...
                # 综合密度和距离计算调整因子
                # 高密度区域：缩小椭球体；低密度区域：扩大椭球体
                # 距离其他簇远的区域：扩大椭球体；距离其他簇近的区域：缩小椭球体
                self.cluster_ellipsoid_adjustments_[cluster_idx] = \
                    self.density_weight * (1 - avg_density) + \
                    self.cluster_distance_weight * norm_distance
            else:
                self.cluster_ellipsoid_adjustments_[cluster_idx] = 1.0  # 默认不调整
    
    def _adjust_ellipsoids_by_density(self):
        """基于聚类和密度调整椭球体大小"""
        # 计算调整因子
        self._calculate_ellipsoid_adjustments()
        
        # 遍历每个分类器的椭球体
        for clsr_idx, ellipsoid_dict in enumerate(self.HEllipsoids):
            # 确保字典包含必要的键
            if "centers" not in ellipsoid_dict or "cov_matrices" not in ellipsoid_dict:
                continue
                
            centers = ellipsoid_dict["centers"]
            cov_matrices = ellipsoid_dict["cov_matrices"]
            
            # 对每个椭球体应用调整
            for ellipsoid_idx in range(len(centers)):
                # 找到距离椭球体中心最近的簇
                if hasattr(self, 'DSEL_data_pca_') and hasattr(self, 'pca_') and self.pca_ is not None:
                    try:
                        # 使用PCA降维后的数据计算
                        ellipsoid_center_pca = self.pca_.transform(centers[ellipsoid_idx].reshape(1, -1))
                        cluster_distances = cdist(ellipsoid_center_pca, self.cluster_centers_)
                    except ValueError:
                        # 如果转换失败，说明椭球体和DSEL_data_的特征数量不匹配
                        ...
                        # 使用原始空间的聚类中心计算距离
                        cluster_distances = cdist(centers[ellipsoid_idx].reshape(1, -1), self.cluster_centers_original_)
                else:
                    # 使用原始数据计算
                    cluster_distances = cdist(centers[ellipsoid_idx].reshape(1, -1), self.cluster_centers_)
                
                nearest_cluster = np.argmin(cluster_distances)
                adjustment_factor = self.cluster_ellipsoid_adjustments_[nearest_cluster]
                
                # 应用调整到协方差矩阵
                # 调整因子越大，椭球体越大
                new_cov = cov_matrices[ellipsoid_idx] * adjustment_factor
                
                # 确保协方差矩阵的正定性和数值稳定性
                new_cov = new_cov + self.reg_lambda * np.eye(self.n_features_)
                
                # 更新协方差矩阵
                cov_matrices[ellipsoid_idx] = new_cov
            
            # 更新HEllipsoids字典
            self.HEllipsoids[clsr_idx]["cov_matrices"] = cov_matrices
    
    def estimate_competence(self, query, neighbors=None, distances=None, predictions=None):
        """基于聚类和密度的竞争力估计"""
        # 调用父类的基本竞争力估计
        basic_competences = super(FHDES_Allboxes_vector_he_clustering_optimized, self).estimate_competence(query, neighbors, distances, predictions)
    ...
    def _calculate_enhanced_competence(self, query):
        ...
        for clsr in range(len(self.HEllipsoids)):
            ...
            # 确保椭球体字典包含必要的键
            if "centers" not in self.HEllipsoids[clsr] or "cov_matrices" not in self.HEllipsoids[clsr]:
                continue
            ...
            if len(centers) > 1:
                ...
                for i in range(n_queries):
                    # 基础隶属度
                    base_membership = clsrEllipsoids_m[b1[i], i] * 0.7 + clsrEllipsoids_m[b2[i], i] * 0.3
                ...
                # 只有一个椭球体的情况
                for i in range(n_queries):
                    competences[i, int(clsr)] = clsrEllipsoids_m[0, i]
