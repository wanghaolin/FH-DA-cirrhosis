...
class FHDES_JFB_vector_he_clustering_optimized(FHDES_JFB_vector_he):
    ...
    def __init__(self, pool_classifiers=None,
        ...
        # 调用父类初始化
        super(FHDES_JFB_vector_he_clustering_optimized, self).__init__(
    ...
    def fit(self, X, y):
        ...
        """
        super(FHDES_JFB_vector_he_clustering_optimized, self).fit(X, y)
        ...
        for clsr_idx, clf in enumerate(self.pool_classifiers_):
            ...
            # 确保数值稳定性
            box_min = np.minimum(box_min, box_max - 1e-8)
            box_max = np.maximum(box_max, box_min + 1e-8)
            center = (box_min + box_max) / 2
            radii = (box_max - box_min) / 2
            cov = np.diag(radii ** 2 + 1e-8)
    ...
    def _ensure_hyperellipsoid_structure(self):
        ...
        for box in self.HBoxes:
            if "Cov" not in box:
                ...
                box["Cov"] = np.eye(n_features) + 1e-8  # 添加正则化项确保正定性
    ...
    def estimate_competence(self, query, neighbors=None, distances=None, predictions=None):
        ...
        basic_competences = super(FHDES_JFB_vector_he_clustering_optimized, self).estimate_competence(
        ...
        for i in range(n_queries):
            ...
            for clsr_idx in range(self.n_classifiers_):
                ...
                for box in clsr_boxes:
                    ...
                    # 计算椭球体中心到各簇中心的距离
                    # 确保center是NumPy数组格式
                    center_array = center.values if hasattr(center, 'values') else center
                    if self.pca_ is not None:
                        center_pca = self.pca_.transform(center_array.reshape(1, -1))[0]
                        box_distances = cdist([center_pca], self.cluster_centers_)[0]
                    else:
                        box_distances = cdist([center_array], self.cluster_centers_)[0]
    ...
    def setup_hyperboxs(self, classifier):
        ...
        hboxV, hboxW = super(FHDES_JFB_vector_he_clustering_optimized, self).setup_hyperboxs(classifier)
