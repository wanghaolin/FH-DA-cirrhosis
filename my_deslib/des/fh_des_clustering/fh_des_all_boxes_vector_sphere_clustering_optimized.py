...
class FHDES_AllBoxes_vector_sphere_clustering_optimized(FHDES_AllBoxes_vector_sphere):
    ...
    def __init__(self, pool_classifiers=None,
        ...
        # 调用父类初始化
        super(FHDES_AllBoxes_vector_sphere_clustering_optimized, self).__init__(
    ...
    def fit(self, X, y):
        # 调用父类fit方法
        super(FHDES_AllBoxes_vector_sphere_clustering_optimized, self).fit(X, y)
    ...
    def _adjust_boxes_by_density(self):
        ...
        for clsr_idx, hbox_dict in enumerate(self.HBoxes):
            # 确保字典包含必要的键
            if "Center" not in hbox_dict or "Radius" not in hbox_dict:
                continue
            
            hboxC = hbox_dict["Center"]
            hboxR = hbox_dict["Radius"]
            
            # 对每个超球应用调整
            for box_idx in range(len(hboxC)):
                # 如果使用了PCA，需要将超球中心也投影到PCA空间
                if hasattr(self, 'DSEL_data_pca_') and hasattr(self, 'pca_') and self.pca_ is not None:
                    try:
                        # 使用PCA降维后的数据计算
                        box_center_pca = self.pca_.transform(hboxC[box_idx].reshape(1, -1))
                        ...
                        # 使用原始空间的聚类中心计算距离
                        cluster_distances = cdist(hboxC[box_idx].reshape(1, -1), self.cluster_centers_original_)
                else:
                    # 使用原始数据计算
                    cluster_distances = cdist(hboxC[box_idx].reshape(1, -1), self.cluster_centers_)
                
                nearest_cluster = np.argmin(cluster_distances)
                adjustment_factor = self.cluster_box_adjustments_[nearest_cluster]
                
                # 应用调整，确保半径不小于最小值
                new_radius = max(hboxR[box_idx] * adjustment_factor, 1e-8)
                
                # 更新超球半径
                hboxR[box_idx] = new_radius
            
            # 更新HBoxes字典
            self.HBoxes[clsr_idx]["Radius"] = hboxR
    
    def estimate_competence(self, query, neighbors=None, distances=None, predictions=None):
        """基于聚类和密度的竞争力估计"""
        # 调用父类的基本竞争力估计
        basic_competences = super(FHDES_AllBoxes_vector_sphere_clustering_optimized, self).estimate_competence(query, neighbors, distances, predictions)
    ...
    def _calculate_enhanced_competence(self, query):
        ...
        for clsr in range(len(self.HBoxes)):
            ...
            # 确保超盒字典包含必要的键
            if "Center" not in self.HBoxes[clsr] or "Radius" not in self.HBoxes[clsr]:
                continue
            ...
            if len(hboxC) > 1:
                ...
                for i in range(n_queries):
                    # 基础隶属度
                    base_membership = clsrSpheres_m[b1[i], i] * 0.7 + clsrSpheres_m[b2[i], i] * 0.3
                ...
                # 只有一个超球的情况
                for i in range(n_queries):
                    competences[i, int(clsr)] = clsrSpheres_m[0, i]
